...
using Settings = QuickDate.AppSettings;
        ...
        public class AudioRecorderAndPlayer
        {
            ...
            public AudioRecorderAndPlayer(string userId)
            {
                try
                {
                    ...
                    SoundFileFullPath = new File(Path.FolderDcimMyApp + "/" + userId + "/" + SoundFile);

                    var dir = Path.FolderDcimMyApp + "/" + userId;
            ...
            public static void InvokeAndShowDialog(Activity activity, string title, string message, string positiveButtonstring)
            {
                try
                {
                    activity.RunOnUiThread(() =>
                    {
                        try
                        {
                            Android.Support.V7.App.AlertDialog.Builder builder = new Android.Support.V7.App.AlertDialog.Builder(activity, Resource.Style.AlertDialogCustom);
                            builder.SetTitle(title);
                            builder.SetMessage(message);
                ...
                if (isKitKat && DocumentsContract.IsDocumentUri(context, uri))
                {
                    ...
                    if (IsExternalStorageDocument(uri))
                    {
                        ...
                        if ("primary".Equals(type, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            return Methods.Path.AndroidDcimFolder + "/" + split[1];
                        }
