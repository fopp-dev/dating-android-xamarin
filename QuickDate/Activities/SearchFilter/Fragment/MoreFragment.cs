...
namespace QuickDate.Activities.SearchFilter.Fragment
{
    public class MoreFragment : Android.Support.V4.App.Fragment, MaterialDialog.IInputCallback, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback
    {
        ...
        {
            var education = ListUtils.SettingsSiteList.FirstOrDefault()?.Education?.FirstOrDefault(a => a.ContainsKey(UserDetails.Education))?.Values.FirstOrDefault();
            IdEducation = string.IsNullOrWhiteSpace(UserDetails.Education) ? 0 : int.Parse(UserDetails.Education);
            EdtEducation.Text = education;

            var pets = ListUtils.SettingsSiteList.FirstOrDefault()?.Pets?.FirstOrDefault(a => a.ContainsKey(UserDetails.Pets))?.Values.FirstOrDefault();
