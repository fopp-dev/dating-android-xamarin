...
namespace QuickDate.Activities.SearchFilter.Fragment
{
    public class LifestyleFragment : Android.Support.V4.App.Fragment, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback
    {
        ...
        {
            var relationship = ListUtils.SettingsSiteList.FirstOrDefault()?.Relationship?.FirstOrDefault(a => a.ContainsKey(UserDetails.RelationShip))?.Values.FirstOrDefault();
            ...
            var smoke = ListUtils.SettingsSiteList.FirstOrDefault()?.Smoke?.FirstOrDefault(a => a.ContainsKey(UserDetails.Smoke))?.Values.FirstOrDefault();
            IdSmoke = string.IsNullOrWhiteSpace(UserDetails.Smoke) ? 0 : int.Parse(UserDetails.Smoke);
            EdtSmoke.Text = smoke;

            var drink = ListUtils.SettingsSiteList.FirstOrDefault()?.Drink?.FirstOrDefault(a => a.ContainsKey(UserDetails.Drink))?.Values.FirstOrDefault();
