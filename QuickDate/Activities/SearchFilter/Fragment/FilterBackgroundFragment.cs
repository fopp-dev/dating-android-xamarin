...
namespace QuickDate.Activities.SearchFilter.Fragment
{
    public class FilterBackgroundFragment : Android.Support.V4.App.Fragment, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback
    {
        ...
        public void SetLocalData()
        {
            ...
            var ethnicity = ListUtils.SettingsSiteList.FirstOrDefault()?.Ethnicity?.FirstOrDefault(a => a.ContainsKey(UserDetails.Ethnicity))?.Values.FirstOrDefault();

            IdEthnicity = string.IsNullOrWhiteSpace(UserDetails.Ethnicity) ? 0 : int.Parse(UserDetails.Ethnicity);

            EdtEthnicity.Text = ethnicity;

            var religion = ListUtils.SettingsSiteList.FirstOrDefault()?.Religion?.FirstOrDefault(a => a.ContainsKey(UserDetails.Religion))?.Values.FirstOrDefault();
