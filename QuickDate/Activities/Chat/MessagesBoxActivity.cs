...
namespace QuickDate.Activities.Chat
{
    ...
    public class MessagesBoxActivity : AppCompatActivity, IOnClickListenerSelectedMessages
    {
        ...
        private void InitKeyboardService()
        {
            keyboardService = new KeyboardService(this, RootView);
        ...
        private void ReplaceButtonFragment(SupportFragment fragmentView)
        {
            try
            {
                keyboardService.HideKeyboardIfShowing(EmojisIconEditTextView);
        ...
        private bool CanDoChat()
        {
            ...
            var totalMessageSent = MAdapter.MessageList.Where(data => data.From == UserDetails.UserId).Count();

            if (AppSettings.ShouldHaveLimitOnMessages && isPro == "0" && totalMessageSent >= GlobalConstants.MaxMessageLimitForNonProUser)
            {
                canChat = false;
